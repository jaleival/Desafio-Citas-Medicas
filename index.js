...
const getUser = async () => {
  ...
  const user = {
    ...
    gender: _.get(data, "results[0].gender"),
    registered: moment(data.results[0].registered.date).format(
...
app.post("/add-user", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: `Usuario ${user.name} agregado correctamente`, user });
  } catch (error) {
...
app.get("/usuarios", (req, res) => {
  try {
    ...
    mujeres.forEach((user, index) => {
      console.log(
        chalk.blue(
          `${index + 1}. Nombre: ${user.name}, Apellido: ${
            user.last_name
          }, ID: ${user.id}, Sexo: ${user.gender}, Timestamp: ${
            user.registered
          }`
        )
      );
    ...
    hombres.forEach((user, index) => {
      console.log(
        chalk.blue(
          `${index + 1}. Nombre: ${user.name}, Apellido: ${
            user.last_name
          }, ID: ${user.id}, Sexo: ${user.gender}, Timestamp: ${user.registered}
          `
        )
      );
    });
